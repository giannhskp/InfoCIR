        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    ...
                    for ext in ['.jpg', '.png']:
                        potential_path = self.dataset_path / 'images' / f"{reference_name}{ext}"
                        if potential_path.exists():
                            reference_image_path = potential_path
                            break
                    
                    if reference_image_path is None:
                        raise FileNotFoundError(f"Could not find image for {reference_name} with .jpg or .png extension")
                    
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    target_name = self.triplets[index]['target']
                    
                    # Try both extensions for target image too
                    target_image_path = None
                    for ext in ['.jpg', '.png']:
                        potential_path = self.dataset_path / 'images' / f"{target_name}{ext}"
                        if potential_path.exists():
                            target_image_path = potential_path
                            break
                    
                    if target_image_path is None:
                        raise FileNotFoundError(f"Could not find image for {target_name} with .jpg or .png extension")
                ...
                image_path = None
                for ext in ['.jpg', '.png']:
                    potential_path = self.dataset_path / 'images' / f"{image_name}{ext}"
                    if potential_path.exists():
                        image_path = potential_path
                        break
                
                if image_path is None:
                    raise FileNotFoundError(f"Could not find image for {image_name} with .jpg or .png extension")
